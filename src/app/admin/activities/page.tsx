                  ...
                  <td className="px-4 md:px-6 py-4">
                    <div className="flex items-center">
                      {activity.image_url ? (
                        ...
                        <img className="h-10 w-10 md:h-12 md:w-12 rounded object-cover flex-shrink-0" src={activity.image_url} alt="" />
                      ) : (
                        <div className="h-10 w-10 md:h-12 md:w-12 rounded bg-gray-200 flex items-center justify-center flex-shrink-0">
                          <span className="text-xs text-gray-500">No Img</span>
                        </div>
                      )}
                      <div className="ml-3 md:ml-4">
                        <div className="text-sm font-medium text-gray-900">{activity.title}</div>
                        <div className="text-xs text-gray-500 line-clamp-1 max-w-[200px] md:max-w-sm">{activity.description}</div>
                        
                        {/* Mobile only date */}
                        <div className="md:hidden mt-1 text-[10px] text-gray-500">
                          Tgl: {activity.date ? new Date(activity.date).toLocaleDateString('id-ID') : '-'}
                        </div>
                      </div>
                    </div>
                  </td>
